...
export function AdviceCard({ initialSuggestions, context }: Props) {
  ...
  function generate() {
    startGenerate(async () => {
      ...
      setOwnAdvice({
        lines: lines.length > 0 ? lines : [isEn ? 'No high-confidence suggestions right now — feel free to continue with your current work.' : '暂时没有高置信建议——继续做你手头的事就好。'],
        mocked: r.mocked,
      });
    ...
    <div className="stack" style={{ gap: 10 }}>
      <div className="row-between" style={{ marginBottom: 4 }}>
        ...
        <button className="btn btn-sm btn-ghost" onClick={generate} disabled={generating}>
          <Icon.sparkles size={13} /> {generating ? (isEn ? 'Generating…' : '生成中…') : (isEn ? 'Generate Today’s Advice' : '生成今日建议')}
        </button>
      </div>
        ...
        <div key={i} className="row" style={{ gap: 8, alignItems: 'flex-start' }}>
          ...
          {!converted.has(i) ? (
            <button
              ...
              disabled={converting}
              title={isEn ? 'Convert to task' : '转为任务'}
            >
              <Icon.plus size={12} />
            </button>
          ) : (
            <span className="badge badge-green" style={{ fontSize: 10 }}>{isEn ? 'Added' : '已加入'}</span>
          )}
        </div>
      ))}
    </div>
