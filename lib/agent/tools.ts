...
import { vetOrigin, learnFromSkeleton, recordScrapeResult, markNeedsLogin, recipeUrl, MAX_RECIPES_PER_WORKSPACE } from '../scrape/recipe';
  ...
  async run(ctx, args) {
    ...
    if (recipeId) {
      const r = await prisma.scrapeRecipe.findFirst({
        ...
        select: { id: true, name: true, status: true, rules: true },
      });
    ...
    const page = await browseLocal(vet.url!, url, rules, waitSec);
    if (!page.ok) {
    ...
    if (recipe && (recipe.status !== 'active' || got === 0)) {
      const learned = await learnFromSkeleton({ tenantId: ctx.tenantId, recipeId: recipe.id, skeleton: page.skeleton });
    ...
    if (recipe) await recordScrapeResult(recipe.id, ctx.workspaceId, got > 0);

    return {
      ok: true,
      data: { title: page.title, values: page.values },
      summary: page.values
        ? `「${page.title}」：${Object.entries(page.values).map(([k, v]) => `${k}=${v}`).join('，')}`
        : `打开了「${page.title}」（没给配方，只读了标题）`,
    };
  },
};
...
const exportScriptTool: AgentTool = {
  ...
  async run(ctx, args) {
    ...
    const r = await prisma.scrapeRecipe.findFirst({
      ...
      select: { name: true, origin: true, pathPattern: true, rules: true, status: true },
    });
    if (!r) return { ok: false, summary: '找不到这个配方' };
