...
//（一句话：这里和四个 tools-*.ts 曾经互相 import，是个靠 `import type` 侥幸没炸的循环）。
//
// 下面这行 re-export 是给**存量 import 点**留的：十来处 `from '@/lib/agent/tools'`
// 拿类型的地方不必改。新代码请直接从 './tool-types' 拿。
export type { ToolContext, ToolResult, ArtifactKind, AgentTool } from './tool-types';
export { DEFAULT_TOOL_TIMEOUT_MS, toolTimeoutMs } from './tool-types';

import {
  type ToolContext, type ToolResult, type AgentTool,
  str, num, clamp,
} from './tool-types';
// ── 读工具 ──────────────────────────────────────────────────────────────────
...
export const AGENT_TOOLS: AgentTool[] = [
  // 本机能力类（命令/文件/采集配方/浏览器驱动）——整机版与桌面版专属，
  // SaaS 形态下每一个都恒返回「这个版本不提供」，见 tools-local.ts 开头
  ...LOCAL_TOOLS,
  listTopics,
  listDrafts,
  readDraft,
