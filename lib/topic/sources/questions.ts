...
// 读者诉求（不是疑问句，但表达了「想看」「求出」的需求——对选题同样有效）。
//
// ⚠️ 这一类的判定在最前面且**无条件返回 true**，所以它比 STRONG/WEAK 更需要克制：
// 词表里放进任何一个「话题名词」或「裸动词」都会当场破掉文件头那条铁律。
// 2026-08-07 实测过一版含「教程 / 更新 / 蹲 / 想看 / 讲一下 / 说一下」的词表，
// 十条纯夸奖评论十条被判成提问——「感谢更新，这期质量真高」「教程收藏了慢慢学」
// 「我蹲了半天终于等到了」全都进了选题池。评论区里这类话恰恰是**最多**的，
// 于是这个功能不是漏掉一些需求，而是把推荐池整个灌成了彩虹屁。
//
// 所以词表只收**祈使/索取**结构（求…、催更、蹲一个、什么时候出、能不能出…），
// 不收话题名词：「教程」本身不是诉求，「求教程」才是（后者已在 STRONG 里）。
const DEMAND = [
  // 「求」类：祈使，几乎不会出现在陈述句里
  '求一期', '求一个', '求个', '求更新', '求出',
  // 催更与蹲：必须带量词/宾语，裸「蹲」会命中「我蹲了半天」
  '催更', '蹲一个', '蹲一期', '蹲个',
  // 追问下一期
  '什么时候出', '什么时候更新', '啥时候出', '啥时候更新',
  // 「能/可以 + 出」
  '能出个', '能出一期', '能不能出', '可以出个', '出一期',
  // 「想看」必须有指向：裸「想看」会命中「想看的内容都齐了」
  '想看你', '想看看', '还想看', '想看更多',
];
...
export function isQuestion(line: string): boolean {
  ...
  if (DEMAND.some((w) => t.includes(w))) return true;
