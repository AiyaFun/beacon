...
export const SCHEDULES: { name: JobName; cron: string; note: string }[] = [
  ...
  // 广播型：待支付订单对账。**必须比每日跑得勤**——它兜的是「钱收了、套餐没发」，
  // 每天才查一次意味着最坏要让用户等 24 小时才拿到自己已经付过钱的东西。
  // 每 10 分钟一轮的代价很小：正常情况下待支付订单为 0，一次查询都不会发出。
  // 写成 `*/10` 而不是 range-step：整机版的本地调度器不认后者（见 tick_agent_runs 那条的说明）。
  { name: 'reconcile_orders', cron: '*/10 * * * *', note: '每 10 分钟对账待支付订单（回调丢了也能发货）' },
