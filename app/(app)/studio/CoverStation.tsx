...
const getRiskLabel = (level: string, lang: string): { text: string; cls: string } | null => {
  const meta: Record<string, { textZh: string; textEn: string; cls: string }> = {
    low: { textZh: '合规风险低', textEn: 'Low Compliance Risk', cls: 'badge-green' },
    medium: { textZh: '合规风险中', textEn: 'Medium Compliance Risk', cls: 'badge-amber' },
    high: { textZh: '合规风险高', textEn: 'High Compliance Risk', cls: 'badge-red' },
  };
  const m = meta[level];
  if (!m) return null;
  return { text: lang === 'en' ? m.textEn : m.textZh, cls: m.cls };
};

function getSpecLabel(key: string | undefined | null, defaultLabel: string, lang: 'zh' | 'en') {
  if (lang === 'en' && defaultLabel) {
    return defaultLabel
      .replace('小红书', 'RED')
      .replace('视频号', 'Channels')
      .replace('抖音', 'Douyin')
      .replace('B站', 'Bilibili')
      .replace('公众号', 'WeChat')
      .replace('横版', 'Widescreen')
      .replace('方图', 'Square');
  }
  return defaultLabel;
}
...
export function CoverStation({
  ...
  async function pick(files: FileList | null, kind: 'subject' | 'background') {
    ...
    if (room <= 0) {
      setRefErr(
        kind === 'subject'
          ? lang === 'en'
            ? `Portrait/subject max ${max} images`
            : `人像/主体最多 ${max} 张`
          : lang === 'en'
          ? `Background max ${max} images`
          : `背景最多 ${max} 张`
      );
      return;
    }
  ...
  async function removeAsset(id: string) {
    if (!window.confirm(lang === 'en' ? 'Delete this image from "My Portraits"? This action is permanent.' : '从「我的形象」里删掉这张？删了就没有了。')) return;
    setBusy('deleting');
  ...
  async function generate(overrides?: { styleKey?: string; reuseMeta?: boolean }) {
    ...
    setPhase({ kind: 'running', hint: lang === 'en' ? 'Submitting…' : '正在提交…', secs: 0 });
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        const j = await res.json().catch(() => ({ error: lang === 'en' ? 'Request failed' : '请求失败' }));
        throw new Error(j.error ?? (lang === 'en' ? 'Request failed' : '请求失败'));
      }
      ...
      if (!ctype.includes('text/event-stream')) {
        throw new Error(
          res.redirected || res.url.includes('/login')
            ? (lang === 'en' ? 'Session expired. Please refresh the page and log in again before generating.' : '登录已过期，刷新页面重新登录后再生成。')
            : (lang === 'en' ? 'Request did not reach service (likely blocked by gateway, often image file is too large). Try a smaller image or retry later.' : '请求没到达服务（多半被网关拦下了，常见于图片太大）。换张小一点的参考图，或稍后重试。'),
        );
      }
      ...
      for (;;) {
        ...
        for (const c of chunks) {
          ...
          if (ev === 'failed') throw new Error(String(data.error ?? (lang === 'en' ? 'Generation failed' : '生成失败')));
          if (ev === 'done') {
            clearInterval(tick);
      ...
      throw new Error(lang === 'en' ? 'Connection interrupted. Please retry (quota will be refunded if deducted without image generated).' : '连接中断，请重试（如果扣了额度而没出图，稍后会自动归还）。');
    } catch (e) {
  ...
  const risk = result ? getRiskLabel(result.riskLevel, lang) : null;
    ...
    <div ref={rootRef} id="cover-station" className="stack" style={{ gap: 12 }}>
      ...
      <div className="row wrap" style={{ gap: 8, alignItems: 'center' }}>
        <select
          ...
          title={lang === 'en' ? 'Aspect ratio selected based on platform; change if needed' : '比例按草稿平台自动选好了，需要别的比例可以换'}
        >
        ...
        <input
          ...
          placeholder={hasContent ? (lang === 'en' ? 'Cover Headline (auto-extracted from body if left blank)' : '封面大字（留空则从正文自动提炼）') : (lang === 'en' ? 'Cover Headline (fill in a hook sentence)' : '封面大字（这稿还没正文，先填一句）')}
          value={mainTitle}
        ...
        <input
          ...
          placeholder={lang === 'en' ? 'Subtitle (optional)' : '副字（可选）'}
      ...
      {titleTooLong && (
        <div className="small" style={{ color: 'var(--amber)', marginTop: -6 }}>
          {lang === 'en'
            ? `Headline is ${titleLen} chars. Exceeding ${COVER_TITLE_SOFT_MAX} chars will crowd the image (won't be auto-truncated)`
      ...
      {!mainTitle.trim() && draftTitle && (
        <div className="row wrap" style={{ gap: 6, alignItems: 'center', marginTop: -6 }}>
          <span className="small muted">{lang === 'en' ? 'Use draft title directly?' : '要不要直接用草稿标题？'}</span>
          <button className="btn btn-xs btn-ghost" onClick={() => onCoverTextChange({ ...coverText, mainTitle: draftTitle.slice(0, COVER_TITLE_HARD_MAX) })}>
            {lang === 'en' ? 'Use "' : '用「'}{draftTitle.slice(0, 18)}{draftTitle.length > 18 ? '…' : ''}{lang === 'en' ? '"' : '」'}
          </button>
        </div>
      )}
      ...
      <div className="row wrap" style={{ gap: 6, alignItems: 'center' }}>
        <span className="small muted" style={{ marginRight: 2 }}>{lang === 'en' ? 'Style' : '风格'}</span>
        {topStyles.map(({ style, recommended }) => (
          ...
          >
            {style.name}
            {recommended && <span className="small" style={{ marginLeft: 4, opacity: 0.8 }}>{lang === 'en' ? '· Rec' : '· 推荐'}</span>}
          </button>
        ))}
        ...
        <button className="btn btn-sm btn-ghost" onClick={() => setShowAllStyles(true)}>
          {lang === 'en' ? `All ${COVER_STYLE_OPTIONS.length + presets.length} Styles ▸` : `全部 ${COVER_STYLE_OPTIONS.length + presets.length} 种 ▸`}
        </button>
        {multiStyles.length > 1 && (
          <span className="small muted">
            {lang === 'en'
              ? `${multiStyles.length} styles selected, generating one for each`
              : `已多选 ${multiStyles.length} 个风格，各出一张`}
            {multiStyles.length > mainBudget
              ? (lang === 'en' ? ` (Only first ${mainBudget} generated: 1:1 sub-cover takes 1 slot)` : `（这次只出前 ${mainBudget} 个：1:1 次图占了一张名额）`)
              : ''}
          </span>
        )}
      </div>
      ...
      <div className="row wrap" style={{ gap: 10, alignItems: 'center' }}>
        <span className="small muted">{lang === 'en' ? 'Portrait / Subject' : '人像/主体'}</span>
        {subject.map((s, i) => (
          <Thumb key={i} src={s.dataUrl} lang={lang} onRemove={() => setSubject((p) => p.filter((_, j) => j !== i))} />
        ))}
        {subject.length < MAX_SUBJECT_IMAGES && (
          <UploadButton
            label={lang === 'en' ? '+ Add Photo' : '+ 添加你的照片'}
            disabled={!consent}
            title={consent ? (lang === 'en' ? 'Portraits yield the highest CTR; optional' : '人像出镜的封面效果最好；不上传也能出图') : (lang === 'en' ? 'Check consent below before uploading' : '先勾选下面的同意确认，再上传照片')}
            onFiles={(f) => pick(f, 'subject')}
          />
        )}
        <span className="small muted" style={{ marginLeft: 6 }}>{lang === 'en' ? 'Background' : '背景'}</span>
        {backgrounds.map((s, i) => (
          <Thumb key={i} src={s.dataUrl} lang={lang} onRemove={() => setBackgrounds((p) => p.filter((_, j) => j !== i))} />
        ))}
        {backgrounds.length < MAX_BACKGROUND_IMAGES && (
          <UploadButton
            label={lang === 'en' ? '+ Background' : '+ 背景图'}
            multiple
            disabled={!consent}
            title={consent ? (lang === 'en' ? 'B-roll / scene images; AI blends or extracts elements' : '空镜 / 场景图，AI 会从中取材或融合') : (lang === 'en' ? 'Check consent below before uploading' : '先勾选下面的同意确认，再上传')}
        ...
        <label className="small muted" style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 6 }}>
          ...
          {lang === 'en' ? 'Textless Version' : '文字留白版'}
        </label>
        ...
        <label className="small muted" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <select
            ...
            title={
              multiStyles.length > 1
                ? (lang === 'en' ? 'When multiple styles selected, count = selected styles' : '多选了风格时，张数 = 选中的风格数')
                : pairing
                  ? (lang === 'en' ? `Paired mode enabled; 1:1 sub-cover takes 1 slot, max ${mainBudget} main covers` : `勾了成对出图，1:1 次图占掉一张，主图最多 ${mainBudget} 张`)
                  : (lang === 'en' ? 'Generate multiple different variants for same style' : '同一风格出几张不同的变体')
            }
          >
            {Array.from({ length: mainBudget }, (_, i) => i + 1).map((n) => (
              <option key={n} value={n}>{n} {lang === 'en' ? 'img' : '张'}</option>
            ))}
          </select>
          {pairing && <span>{lang === 'en' ? 'Main' : '主图'}</span>}
        </label>
        {specKey === WECHAT_MAIN_SPEC && (
          <label className="small muted" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <input type="checkbox" checked={wechatSquareToo} onChange={(e) => setWechatSquareToo(e.target.checked)} />
            {lang === 'en' ? 'Include 1:1 square sub-cover (WeChat requires two)' : '连 1:1 次图一起出（公众号要两张）'}
          </label>
        )}
        <button className="btn btn-sm btn-ghost" onClick={() => setShowMore(true)}>
          <Icon.settings size={13} /> {lang === 'en' ? 'More Options' : '更多选项'}
          {(fontKey !== DEFAULT_COVER_FONT || decors.length > 0 || extra.trim()) ? (lang === 'en' ? ' (Configured)' : '（已设置）') : ''}
        </button>
      </div>
      ...
      {(library.length > 0 || subject.length + backgrounds.length > 0) && (
        <div className="row wrap" style={{ gap: 8, alignItems: 'center' }}>
          {library.length > 0 && <span className="small muted">{lang === 'en' ? 'My Portraits' : '我的形象'}</span>}
          {library.map((a) => {
            const on = pickedAssets.includes(a.id);
            const kindLabel = a.kind === 'portrait' ? (lang === 'en' ? 'Portrait' : '人像') : a.kind === 'background' ? (lang === 'en' ? 'Background' : '背景') : (lang === 'en' ? 'Brand' : '品牌元素');
            return (
              <span key={a.id} style={{ position: 'relative', display: 'inline-block' }}>
                <button
                  onClick={() => togglePickedAsset(a.id)}
                  title={`${kindLabel}${a.label ? ' · ' + a.label : ''}${lang === 'en' ? ' (Click to use)' : '（点一下这次用它）'}`}
                  style={{
                    padding: 0,
                ...
                >
                  ...
                  <img src={a.url} alt={a.label ?? (lang === 'en' ? 'Portrait asset' : '形象素材')} style={{ width: 44, height: 44, objectFit: 'cover', borderRadius: 5 }} />
                </button>
                <button
                  className="btn btn-xs"
                  style={{ position: 'absolute', top: -6, right: -6, padding: '0 5px', lineHeight: 1.4 }}
                  title={lang === 'en' ? 'Delete from My Portraits' : '从我的形象里删掉'}
          ...
          {subject.length + backgrounds.length > 0 && (
            <label className="small muted" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {lang === 'en' ? 'Save uploaded to "My Portraits" (reuse without re-uploading)' : '把刚上传的存进「我的形象」（下次直接勾，不用重传）'}
            </label>
          )}
        </div>
      )}
        ...
        <button
          ...
          title={
            !draftId
              ...
              : !quota.configured
                ? (lang === 'en' ? 'No image generation channel available' : '还没有可用的生图渠道')
                : hasRefs && !consent
                  ? (lang === 'en' ? 'Check consent agreement before generating with reference images' : '上传了照片就需要先勾选同意确认')
                  : undefined
          }
        >
          <Icon.sparkles size={14} /> {running ? (lang === 'en' ? 'Generating…' : '生成中…') : (lang === 'en' ? 'Generate Cover' : '生成封面')}
        </button>
        {quota.configured ? (
          <span className="small muted">
            {lang === 'en'
              ? `Generating ${plannedImages} images${pairedSquare ? ` (${plannedImages - 1} main + 1 1:1 sub-cover)` : ''} · Approx. ${slots} AI quota${needsDerive ? ' (incl. 1 copy extraction)' : ''} · ${remaining} remaining today`
              : `本次出 ${plannedImages} 张${pairedSquare ? `（${plannedImages - 1} 张主图 + 1 张 1:1 次图）` : ''} · 约占 ${slots} 个 AI 名额${needsDerive ? '（含提炼文案 1 次）' : ''} · 今日还能出 ${remaining} 张`}
          </span>
        ) : (
          <span className="small muted">
            {lang === 'en' ? (
              <>
                No image generation channel available — go to{' '}
                <Link href="/settings/keys" style={{ textDecoration: 'underline' }}>Connections & Keys</Link> to add Volcengine Doubao.
              </>
            ) : (
              <>
                还没有可用的生图渠道——到{' '}
        ...
        <div className="card" style={{ padding: 14, boxShadow: 'none', background: 'var(--surface-2)' }}>
          <div className="row-between wrap" style={{ gap: 8, marginBottom: 8 }}>
            <div className="row wrap" style={{ gap: 6, alignItems: 'center' }}>
              <b className="small">
                {lang === 'en'
                  ? `${getSpecLabel(result.spec.key, result.spec.label, lang)} Cover`
                  : `${result.spec.label} 封面`}
              </b>
              ...
              {result.mocked && (
                <span className="badge badge-amber" title={lang === 'en' ? 'Demo model used for title extraction; image is real' : '抽标题那步用的是演示模型；图是真的'}>
                  {lang === 'en' ? 'Demo Copy' : '文案为演示结果'}
                </span>
              )}
            ...
            <span className="small muted">
              {lang === 'en'
                ? 'On-image "AI Generated" badge = explicit notice; implicit AIGC metadata embedded'
                : '图上「AI生成」角标 = 显式标识；文件内已写入隐式 AIGC 标识'}
              {result.images.some((i) => !i.aigcEmbedded)
                ? (lang === 'en' ? ' (Some formats unsupported)' : '（部分格式无法写入）')
                : ''}
            </span>
          </div>
          {!textless && result.meta?.mainTitle && (
            <div className="small muted" style={{ marginBottom: 8 }}>
              {lang === 'en' ? 'Cover Headline: ' : '封面文案：'}<b>{result.meta.mainTitle}</b>
            ...
            {result.images.map((img, i) => (
              <div key={i} className="stack" style={{ gap: 6 }}>
                ...
                <img
                  src={img.url}
                  alt={lang === 'en'
                    ? `${img.specLabel ?? getSpecLabel(result.spec.key, result.spec.label, lang)} Cover`
                    : `${img.specLabel ?? result.spec.label} 封面`}
                  style={{
                ...
                <div className="row wrap" style={{ gap: 6 }}>
                  ...
                  >
                    <Icon.download size={13} /> {lang === 'en' ? 'Download' : '下载'}
                  </button>
                  {img.id && draftId && (
                    <button
                      ...
                      title={lang === 'en' ? 'Selected as cover for this draft (selected covers are protected from retention cleanup)' : '选定为这篇稿子的封面（选定的不会被保留期清理掉）'}
                    >
                      {coverAssetId === img.id
                        ? (lang === 'en' ? '✓ Active Cover' : '✓ 本稿封面')
                        : (lang === 'en' ? 'Set as Draft Cover' : '设为本稿封面')}
                    </button>
                  )}
                </div>
              </div>
          ...
          {result.hits.length > 0 && (
            <>
              ...
              <div className="small muted" style={{ marginBottom: 6 }}>
                {lang === 'en' ? 'Words in headline recommended for review before publishing:' : '封面文案里这些词发布前建议再斟酌：'}
              </div>
          ...
          <div className="row wrap" style={{ gap: 8, alignItems: 'center' }}>
            <button
              ...
              onClick={() => generate({ reuseMeta: true })}
              disabled={running}
              title={lang === 'en' ? 'Generate another with same copy & settings (consumes 1 image credit)' : '同一套文案与设置再出一张（只花一次出图）'}
            >
              <Icon.refresh size={13} /> {lang === 'en' ? 'Reroll' : '换一张'}
            </button>
            <span className="small muted">{lang === 'en' ? 'Try another style:' : '换风格再来一张：'}</span>
            {ranked.slice(0, 5).filter((s) => s.style.key !== styleKey).map(({ style }) => (
              <button
                key={style.key}
                className="btn btn-xs btn-ghost"
                onClick={() => generate({ styleKey: style.key, reuseMeta: true })}
                disabled={running}
                title={style.hint}
              >
                {style.name}
            ...
            <button className="btn btn-xs btn-ghost" onClick={() => setShowAllStyles(true)} disabled={running}>
              {lang === 'en' ? 'More…' : '更多…'}
            </button>
          </div>
        </div>
      ...
      {covers.length > 0 && (
        <div className="stack" style={{ gap: 8 }}>
          <div className="row wrap" style={{ gap: 8, alignItems: 'center' }}>
            <b className="small">{lang === 'en' ? `Draft Covers (${covers.length})` : `本稿封面（${covers.length}）`}</b>
            <span className="small muted">
              {lang === 'en' ? 'Recent generations saved here; persists across page refresh' : '最近生成的都在这里，刷新页面也还在'}
            </span>
          </div>
            ...
            {covers.map((c) => {
              ...
              return (
                <div key={c.id} className="stack" style={{ gap: 4, width: 108 }}>
                  ...
                  <img
                    src={c.url}
                    alt={String(c.meta.mainTitle ?? (lang === 'en' ? 'Cover' : '封面'))}
                    style={{
                      width: 108,
                      height: 108,
                      objectFit: 'cover',
                  ...
                  <div className="small muted" style={{ fontSize: 11, lineHeight: 1.4, minHeight: 15 }}>
                    {isCurrent ? (lang === 'en' ? 'Active Cover' : '本稿封面') : String(c.meta.mainTitle ?? '').slice(0, 10)}
                  </div>
                  <div className="row wrap" style={{ gap: 4 }}>
                    ...
                    {draftId && !isCurrent && (
                      <button
                        className="btn btn-xs btn-ghost"
                        onClick={() => void setAsDraftCover(c.id)}
                        disabled={busy === 'setting'}
                      >
                        {lang === 'en' ? 'Set as Cover' : '设为封面'}
                      </button>
                    )}
                  </div>
                </div>
      ...
      {/* 更多选项弹层：字体倾向 / 装饰 / 备注 */}
      {showMore && (
        <Overlay label={lang === 'en' ? 'More Cover Options' : '封面更多选项'} onClose={() => setShowMore(false)}>
          <div className="card" style={{ padding: 20, background: 'var(--surface)', width: 520, maxWidth: '94vw', maxHeight: '88vh', overflowY: 'auto' }}>
            <div className="row-between" style={{ marginBottom: 12 }}>
              <b className="small">{lang === 'en' ? 'More Options' : '更多选项'}</b>
              <button className="btn btn-xs btn-ghost" onClick={() => setShowMore(false)}>
                {lang === 'en' ? 'Close' : '关闭'}
              </button>
            </div>
            <div className="field">
              <label className="field-label">{lang === 'en' ? 'Typography Sentiment' : '字体倾向'}</label>
              <div className="row wrap" style={{ gap: 6 }}>
                {COVER_FONTS.map((f) => (
                  <button
                    key={f.key}
                    className={`btn btn-sm ${fontKey === f.key ? 'btn-accent' : 'btn-ghost'}`}
                    onClick={() => setFontKey(f.key)}
                    title={f.prompt || (lang === 'en' ? 'Determined by style' : '由风格决定')}
                  >
                    {f.name}
                  </button>
                ))}
              ...
              <div className="small muted" style={{ marginTop: 6 }}>
                {lang === 'en'
                  ? 'This describes typography aesthetics to the AI model rather than using actual font files — final typography depends on generation.'
                  : '这是给 AI 的「字体感」描述，不是真字体文件——所以这里不做预览，实际字形以出图为准。'}
              </div>
            </div>
            <div className="field" style={{ marginTop: 12 }}>
              <label className="field-label">{lang === 'en' ? 'Decorations (Multi-select)' : '装饰点缀（可多选）'}</label>
              <div className="row wrap" style={{ gap: 6 }}>
                {COVER_DECORS.map((d) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => setDecors((p) => (on ? p.filter((x) => x !== d.key) : [...p, d.key]))}
                      title={d.prompt}
                    >
                      {d.name}
                    </button>
            ...
            <div className="field" style={{ marginTop: 12 }}>
              <label className="field-label">{lang === 'en' ? 'Additional Prompt Instructions (Optional)' : '给 AI 的其他要求（可选）'}</label>
              <textarea
                className="input"
                rows={3}
                placeholder={lang === 'en' ? 'E.g.: Coffee shop background / Cool color tone / Subject on the right' : '比如：背景要有咖啡馆 / 整体偏冷色 / 人物放右边'}
                value={extra}
      ...
      {/* 全部风格弹层 */}
      {showAllStyles && (
        <Overlay label={lang === 'en' ? 'Select Cover Style' : '选择封面风格'} onClose={() => setShowAllStyles(false)}>
          <div className="card" style={{ padding: 20, background: 'var(--surface)', width: 720, maxWidth: '94vw', maxHeight: '88vh', overflowY: 'auto' }}>
            <div className="row-between" style={{ marginBottom: 12 }}>
              <b className="small">{lang === 'en' ? 'Cover Styles' : '封面风格'}</b>
              <button className="btn btn-xs btn-ghost" onClick={() => setShowAllStyles(false)}>
                {lang === 'en' ? 'Close' : '关闭'}
              </button>
            </div>
            <div className="small muted" style={{ marginBottom: 10, lineHeight: 1.6 }}>
              {lang === 'en'
                ? `Select a style to generate. Check boxes on top-right to compare multiple styles (up to ${MAX_COVER_IMAGES}, generates 1 each).`
                : `点一个 = 用它出图。想一次比几种，勾右上角的方框多选（最多 ${MAX_COVER_IMAGES} 个，各出一张）。`}
            </div>

            {presets.length > 0 && (
              <>
                <b className="small" style={{ display: 'block', marginBottom: 6 }}>{lang === 'en' ? 'My Custom Styles' : '我的风格'}</b>
                    ...
                    return (
                      ...
                      >
                        <div className="row-between" style={{ gap: 8, alignItems: 'flex-start' }}>
                          ...
                          <button
                            className="btn btn-xs btn-ghost"
                            title={lang === 'en' ? 'Delete this style' : '删掉这个风格'}
                            onClick={async () => {
                              if (!window.confirm(lang === 'en' ? `Delete style "${p.name}"?` : `删掉「${p.name}」？`)) return;
            ...
            <b className="small" style={{ display: 'block', marginBottom: 6 }}>{lang === 'en' ? 'Built-in Styles' : '内置风格'}</b>
                  ...
                  >
                    <div className="row-between" style={{ gap: 8, alignItems: 'flex-start' }}>
                        ...
                        <div className="row wrap" style={{ gap: 6, alignItems: 'center' }}>
                          ...
                          {recommended && (
                            <span className="badge badge-brand" style={{ fontSize: 10 }}>
                              {lang === 'en' ? 'Recommended for Your Niche' : '按你的赛道推荐'}
                            </span>
                          )}
                          {style.portraitFriendly && (
                            <span className="badge badge-gray" style={{ fontSize: 10 }}>
                              {lang === 'en' ? 'Portrait Friendly' : '适合人像'}
                            </span>
                          )}
                        </div>
                      ...
                      <input
                        type="checkbox"
                        checked={checked}
                        title={lang === 'en' ? 'Compare styles: checked items will generate 1 each' : '一次比几种：勾上的各出一张'}
                        onChange={(e) =>
                          setMultiStyles((prev) =>
                            e.target.checked
            ...
            <div className="divider" />
            <b className="small" style={{ display: 'block', marginBottom: 6 }}>{lang === 'en' ? 'Create a Custom Style' : '自己写一个风格'}</b>
            <div className="stack" style={{ gap: 8 }}>
              <input
                className="input"
                placeholder={lang === 'en' ? 'Style name, e.g. "My Dark Gold Theme"' : '风格名字，比如「我的黑金系列」'}
                value={newStyleName}
                maxLength={20}
              ...
              <textarea
                ...
                rows={3}
                placeholder={lang === 'en'
                  ? 'Visual details: colors, background, typography layout, mood. The more specific, the closer the result.'
                  : '画面长什么样：配色、背景、字怎么排、什么氛围。写得越具体，出图越像你要的。'}
                value={newStyleDesc}
                maxLength={600}
              ...
              <div className="row wrap" style={{ gap: 8, alignItems: 'center' }}>
                ...
                >
                  {lang === 'en' ? 'Save to My Styles' : '存进我的风格'}
                </button>
                {styleErr && <span className="small" style={{ color: 'var(--red)' }}>{styleErr}</span>}
                <span className="small muted">
                  {lang === 'en' ? 'Your description will be passed directly to the image model without rewriting.' : '你写的这段会原样发给出图模型，不会被改写。'}
                </span>
              </div>
            </div>
            ...
            {result && (
              <div className="small muted" style={{ marginTop: 10 }}>
                {lang === 'en' ? 'With existing results, clicking a style will immediately reroll with the same headline copy.' : '已有结果时，点一个风格会直接用同一套文案再出一张。'}
              </div>
            )}
          </div>
        </Overlay>
...
function Thumb({ src, onRemove, lang = 'zh' }: { src: string; onRemove: () => void; lang?: 'zh' | 'en' }) {
  return (
    <span style={{ position: 'relative', display: 'inline-block' }}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={src} alt={lang === 'en' ? 'Reference Image' : '参考图'} style={{ width: 44, height: 44, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--border)' }} />
      <button
        className="btn btn-xs"
        style={{ position: 'absolute', top: -6, right: -6, padding: '0 5px', lineHeight: 1.4 }}
