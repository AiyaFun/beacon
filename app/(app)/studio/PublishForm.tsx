...
export function PublishForm({ draftId }: { draftId: string }) {
  ...
  function submit() {
    start(async () => {
      ...
      if (!r.ok) {
        setMsg(r.error ?? (lang === 'en' ? 'Failed to log publish' : '登记失败'));
        return;
      }
      setOpen(false);
      setClosedMsg(r.warning ? (lang === 'en' ? `Logged, but ${r.warning}` : `已登记，但${r.warning}`) : (lang === 'en' ? 'Publish logged' : '已登记发布'));
      setUrl('');
  ...
  const formTitle = lang === 'en' ? 'Log Publish' : '登记发布';

  return (
    <Overlay label={formTitle} onClose={() => setOpen(false)} closable={!pending}>
      <div className="card" style={{ width: 420, maxWidth: '92vw', padding: 24 }}>
        <h3 style={{ margin: '0 0 16px' }}>{formTitle}</h3>

        <label className="small" style={{ display: 'block', marginBottom: 4 }}>
          {lang === 'en' ? 'Post URL (Optional)' : '发布链接（可选）'}
        </label>
        <input
          className="input"
          placeholder={lang === 'en' ? 'Paste published post link for automatic data tracking' : '粘贴发布后的作品链接，用于自动回流数据'}
          value={url}
          ...
          <div className="small" style={{ marginTop: 4, marginBottom: 8 }}>
            {parseResult.ok ? (
              <span style={{ color: 'var(--green)' }}>
                {lang === 'en' ? `Identified as ${platformName(parseResult.platform)} post, analytics will be tracked automatically` : `识别为 ${platformName(parseResult.platform)} 作品，可自动回流数据`}
              </span>
            ) : (
        ...
        <label className="small" style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer' }}>
          ...
          <span>
            {lang === 'en'
              ? `I confirm that platform AI disclosures have been completed, and acknowledge this content is labeled with "${AIGC_LABEL}".`
              : `我确认已在发布平台完成 AI 使用声明，并知悉本内容将标注「${AIGC_LABEL}」（依据《人工智能生成合成内容标识办法》）`}
          </span>
        </label>
        ...
        <div className="row" style={{ gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
          ...
          >
            {pending ? (lang === 'en' ? 'Logging…' : '登记中…') : (lang === 'en' ? 'Confirm Log' : '确认登记')}
          </button>
        </div>
      </div>
    </Overlay>
