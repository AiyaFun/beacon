...
function relTime(iso: string, lang = 'zh'): string {
...
export function AutomationCard({ config, lastRuns }: { config: AutomationConfig; lastRuns: Record<string, LastRun> }) {
  ...
  function runNow(job: string) {
    setRunMsg((m) => ({ ...m, [job]: isEn ? 'Running…' : '运行中…' }));
    start(async () => {
  ...
  return (
    <div className="stack" style={{ gap: 14 }}>
            ...
            <div className="row-between" style={{ alignItems: 'flex-start', gap: 12 }}>
              <div className="stack" style={{ gap: 3 }}>
                <div className="row" style={{ gap: 8, alignItems: 'center' }}>
                  <span style={{ fontWeight: 600 }}>{isEn ? (it.labelEn ?? it.label) : it.label}</span>
                  {it.advanced && <span className="badge badge-gray" style={{ fontSize: 11 }}>{isEn ? 'Advanced' : '高级'}</span>}
                </div>
                <span className="small muted" style={{ lineHeight: 1.5 }}>{isEn ? (it.descEn ?? it.desc) : it.desc}</span>
                {last && (
                  <span className="small muted">
                    {isEn ? 'Last run: ' : '上次运行：'}
                    <span style={{ color: last.status === 'ok' ? 'var(--green)' : last.status === 'failed' ? 'var(--red)' : 'var(--text-3)' }}>
                      {last.status === 'ok' ? (isEn ? 'Success' : '成功') : last.status === 'failed' ? (isEn ? 'Failed' : '失败') : last.status}
                    </span>
                    {last.detail ? ` · ${last.detail}` : ''} · {relTime(last.at, lang)}
                  </span>
                )}
              ...
              <div className="row" style={{ gap: 8, alignItems: 'center', flexShrink: 0 }}>
                {runnable && (
                  <button className="btn btn-sm btn-ghost" disabled={pending} onClick={() => runNow(it.job as string)}>
                    {isEn ? 'Run Now' : '立即运行'}
                  </button>
                )}
                <button
                  ...
                  title={on ? (isEn ? 'Click to disable' : '点击关闭') : (isEn ? 'Click to enable' : '点击开启')}
                  style={{
                    width: 42,
                    height: 24,
                    borderRadius: 12,
      ...
      <div className="small muted" style={{ lineHeight: 1.6 }}>
        {isEn
          ? 'Disabling any automation task will not delete existing data or disconnect integrations; re-enabling restores functionality. Platform-wide shared tasks such as hotlist scraping are not listed here (individual toggles have no effect).'
