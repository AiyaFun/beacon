...
<ul>
  ...
  <li><strong>不读取、不存储、不上传任何平台的 Cookie 或登录凭证</strong>，不代替用户登录，
      不发布、不修改、不删除任何内容。
      <strong>某一页没采到时的处理</strong>：当扩展为采集而在后台打开的那一页什么都没读到时
      （可能是登录墙、页面改版，也可能只是没加载完），扩展不会悄悄把它关掉，而是把这个
      已经打开的页面<strong>切到前台</strong>交给用户本人看一眼。若需要登录，登录动作全部由用户完成——
      扩展<strong>不代替用户登录</strong>（不填写账号密码、不点击登录按钮、不处理二维码）；
      切到前台后该页由用户自行处置，扩展不再自动关闭它。
      此行为只在用户当场点击采集时发生，定时任务不会弹出任何前台页面。
      <strong>扩展不调用任何平台接口。</strong>（2026-09-03 前曾有唯一的例外：公众号<strong>竞对</strong>采集，
      它在用户本人已登录的公众号后台内调用后台自带的查询接口去查<strong>别人的</strong>公众号。
      那不是微信官方开放的接口，风险落在用户自己的公众号账号上，因此该通道已<strong>整条移除</strong>，
      并且不会恢复。）
      <div style="margin-top:8px"><strong>公众号后台的自有数据回填</strong>：读<strong>用户自己</strong>后台里
      <strong>用户自己</strong>作品的数字，与视频号/抖音/小红书/B站四个创作者后台同一条通道、同一套约束
      （只读已渲染页面、不点击、不调接口、不碰 Cookie、不代替登录）。两点不同：
      <code>mp.weixin.qq.com</code> 不在扩展的安装权限里，须由用户在设置页<strong>单独授权</strong>，
      没授权则一行都不注入；且它是官方发行版专有的可选模块，开源发行版不含它。</div></li>
...
<table>
  ...
  <tr><td><code>alarms</code></td>
      <td>① 每日定时刷新图标角标；② 每 12 小时查一次扩展是否有新版本（只收发版本号与公开下载地址，不含用户数据）；
          ...
          ④ 用户显式开启后的每日<strong>自有后台</strong>回填（默认关闭；视频号 / 抖音 / 小红书 / B站内置，公众号须单独授权；只开用户登记了账号的后台）；
  ...
  <tr><td><code>scripting</code></td>
      <td>用户当场点击时，把一个随安装包分发的脚本注入当前标签页读取一次页面内容（读正文 / 读封面文案 / 读已显示的评论）。不常驻、不轮询。</td></tr>
...
<strong>写死在扩展代码里的白名单</strong>，目前有四件：采一个用户已订阅的竞对公开主页
（微博 / 快手 / 知乎 / 头条号 / 百家号五个平台的主页按<strong>内置配方</strong>读：规则由服务端从脱敏骨架学出并下发，
首次需用户对该站点授权一次，安装权限不含这些域名；只补已订阅竞对、不建档）、
回填用户本人创作者后台的数据（视频号 / 抖音 / 小红书 / B站；公众号须先在扩展设置页授权）、
打开用户本人在 X / TikTok / YouTube 上的公开主页回填自己作品的数据、以及「打开指定网址并读取正文」。
