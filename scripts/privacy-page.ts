...
<ul>
  ...
  <li><strong>不读取、不存储、不上传任何平台的 Cookie 或登录凭证</strong>，不代替用户登录，
      ...
      <strong>扩展不调用任何平台接口。</strong>（2026-09-03 前曾有唯一的例外：公众号竞对采集，
      它在用户本人已登录的公众号后台内调用后台自带的查询接口。那不是微信官方开放的接口，
      风险落在用户自己的公众号账号上，因此该通道已<strong>整条移除</strong>——扩展现在不再匹配
      mp.weixin.qq.com 下的任何页面。）</li>
