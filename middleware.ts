...
const PUBLIC_PATHS = [
  ...
  // ⚠️ /indexnow-key.txt：IndexNow 协议的密钥文件。引擎收到我们的提交后**会来读它**，
  //    比对一致才认这次提交。被 307 到登录页 = 比对失败 = 推送静默无效
  //    （推送接口仍然回 200，日志上看是成功的）。这与 /llms.txt 那次是同一条教训。
  '/indexnow-key.txt',
  '/api/auth/local',
  ...
  '/robots.txt',
  '/sitemap.xml',
  // ⚠️ /llms.txt：给大模型看的站点说明（与上面两个同类，都是**只有机器会来读**的公开文件）。
  //    2026-08-29 加它时**漏了这一行**，生产上直接被 307 到登录页——
  //    而本地单测全绿（它们不过 middleware）。与「加 ingest 路由必须同步改 middleware」
  //    是同一条教训，只是换了个入口：**凡是新增对外公开的文件路由，都要同步加进这里**。
  '/llms.txt',
  ...
  '/api/ingest/tasks',
  // 桌面客户端执行器取解析脚本（2026-09-03）：同一把采集令牌鉴权，壳里没有登录态
  '/api/ingest/executor',
