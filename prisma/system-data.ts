...
export const BUILTIN_SKILLS: SystemSkill[] = [
  ...
  {
    slug: 'teleprompter-storyboard',
    name: '提词稿 + 分镜',
    description: '把一篇正文变成能直接对着念的提词稿与逐镜分镜表：每页 ≤ 40 字、标注停顿与画面/字幕/素材，选 9:16 竖屏或 16:9 横屏。录制本身在桌面端做，这里只出稿。',
    emoji: '🎥',
    category: 'generate',
    platform: 'generic',
    outputKind: 'markdown',
    promptTemplate: [
      '把下面的正文改写成一份「提词稿 + 分镜表」，给作者对着摄像头录一条 1–3 分钟的视频用。',
      '【素材】主题：{{title}}',
      '{{persona}}',
      '{{context}}',
      '{{brief}}',
      ...
      '{{content}}',
      '【输出结构（markdown）】',
      '1. 一行写明建议画幅：`画幅：9:16 竖屏` 或 `画幅：16:9 横屏`（口播/情绪向选竖屏，演示/教程选横屏，并说一句为什么）。',
      '2. `## 提词稿`：按页分段，每页一个 `### 第 N 页`，每页正文不超过 40 个字（提词器一屏能看完），句末用 [停顿]、[重音] 标注；开头 3 秒必须是钩子。',
      '3. `## 分镜表`：一个 markdown 表格，列：镜号 | 对应提词页 | 画面（拍什么/怎么拍） | 屏幕字幕 | 需要的素材（截图/B-roll/图表，没有写「无」）| 时长（秒）。',
      '4. `## 录前清单`：3–5 条，只写这条视频真需要准备的东西。',
      '【硬要求】不改变原文观点；不加原文没有的事实；语感照人设与原文样本来，不要大模型套话；只输出 markdown 本身，不加解释。',
    ].join('\n'),
  },
  {
    slug: 'xhs-format',
