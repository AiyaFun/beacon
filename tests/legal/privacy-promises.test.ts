...
// 2026-08-18 改了行为：采集没采到时不再当场关页，而是把那一页切到前台交给用户本人处置
//（他可能需要登录，也可能只是页面没加载完）。这动了「后台标签页、不抢占焦点、采完立即关闭」
// 那条既有承诺，三份政策必须同步说清楚，而且要说清**边界**——否则「插件会把页面弹到你面前」
// 听起来就像它可以随时抢你的屏幕。
//（2026-09-03：公众号那条「等你扫码最长 5 分钟」的通道随公众号采集一并删除，
//  留下的是通用的「没采到就把这一页交还给你」，它没有等待计时器。）
describe('没采到 → 把那一页切到前台：三份政策与代码必须一致', () => {
  it('代码事实：只在 interactive（用户当场点击）时切前台，且切过去的页不再自动关掉', () => {
    expect(SW, '切前台不再受 interactive 约束了？那定时任务会在用户不在时弹页面')
      .toMatch(/opts\.interactive && !stranded && tab\?\.id/);
    // 切到前台的页不许再被自动关掉——政策里写了「归用户所有」
    expect(SW).toMatch(/if \(!keep && tab && tab\.id\) await chrome\.tabs\.remove/);
    // 公众号那条步进机连同它的登录等待一起删干净了
    expect(SW, '插件里还留着 mp.weixin.qq.com').not.toMatch(/mp\.weixin\.qq\.com/);
  });

  it('🔒 三份政策都写明「切到前台 + 只在当场点击时 + 不代替登录」', () => {
    for (const [name, text] of ALL_POLICIES) {
      expect(text, `${name} 没披露「没采到时把页面切到前台」这个会抢焦点的行为`).toContain('切到前台');
      expect(
